...
22. **Grounding por afirmação (ATIVO desde 2026-08-27)** — dúvida técnica gerada só é entregue após cobertura determinística, suficiência/conflito, JSON estrito, IDs autorizados, checagem de números e entailment final de **todas** as afirmações. Qualquer falha bloqueia o texto inteiro e usa abstinência/handoff. Provas sem rede: `evidence-grounding.service.spec.ts`, `rag.service.spec.ts`, `reranker.port.spec.ts`, `retrieval-plan.spec.ts` e o caminho integrado no `ai-response.worker.spec.ts`. A comparação empírica entre provedores continua sendo gate de homologação operacional, não algo simulável no CI sem endpoints aprovados.
