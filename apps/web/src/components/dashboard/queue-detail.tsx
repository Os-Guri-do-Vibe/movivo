...
import { ArrowLeft, CheckCircle2, Clock3, RefreshCw, ShieldAlert } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-4">
        {protocol.content.sessions.map((session) => (
          <details
...
export function QueueDetail({ kind, id }: { kind: QueueKind; id: string }) {
  ...
  return (
    <article>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="flex items-center gap-2 font-mono text-xs text-muted-foreground">
              ...
              {detail.item.severity === 'SAFETY' ? 'SEGURANÇA · PRIORIDADE' : detail.item.kind}
            </p>
            <h1 className="mt-2 text-h1 font-bold">{detail.item.title}</h1>
            <p className="mt-2 max-w-3xl text-body text-muted-foreground">{detail.item.summary}</p>
          </div>
          <span className="rounded-md border border-border px-2 py-1 font-mono text-xs text-muted-foreground">
            {detail.item.status}
          </span>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]">
          ...
          {kind === 'HANDOFF' || kind === 'CHECKIN' ? (
            ...
            >
              ...
              <div className="mt-4">
                <ConfirmAction
                  triggerLabel={
                    kind === 'CHECKIN' ? 'Resolver sinalização' : 'Marcar como resolvido'
                  }
                  title={
                    kind === 'CHECKIN'
