  ...
  return (
    <div>
      ...
      <section
        ...
        aria-labelledby="realized-result"
      >
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          Realizado · mês corrente
        </p>
        <h2 id="realized-result" className="mt-2 font-mono text-h1 font-bold">
          {formatMetric(data.profit)}
        </h2>
      ...
      {projection.status === 'UNAVAILABLE' ? (
        <div className="mt-6">
          <EmptyState
            ...
            description={projection.reason ?? 'Base histórica insuficiente.'}
          />
        </div>
      ) : (
        <>
          <section
            className="mt-6 rounded-xl border border-border bg-secondary p-5"
            aria-labelledby="projection-basis"
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Projetado · não realizado
            </p>
            <h2 id="projection-basis" className="mt-2 text-h2 font-bold">
              Base de cálculo
            </h2>
            <p className="mt-2 text-label text-muted-foreground">{projection.method}</p>
            <p className="mt-2 text-xs text-muted-foreground">
              Meses fechados usados: {projection.basisMonths.map(monthLabel).join(', ')}. Horizonte:{' '}
              {projection.horizonMonths} meses.
            </p>
          </section>
            ...
            {projection.scenarios.map((scenario) => (
              ...
              >
                ...
                <p className="mt-3 font-mono text-h1 font-bold">
                  {brl.format(scenario.totalResultBrl)}
                </p>
                <p className="text-xs text-muted-foreground">
                  Resultado acumulado projetado em 3 meses
                </p>
                <dl className="mt-4 grid grid-cols-2 gap-3 text-label">
                  <div>
                    <dt className="text-xs text-muted-foreground">Receita projetada</dt>
                    <dd className="font-mono font-semibold">
                      {brl.format(scenario.totalRevenueBrl)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-muted-foreground">Custo projetado</dt>
                    <dd className="font-mono font-semibold">{brl.format(scenario.totalCostBrl)}</dd>
                  </div>
                </dl>
                <ul className="mt-4 grid gap-2">
                  {scenario.months.map((month) => (
                    <li
                      key={month.month}
                      className="flex items-center justify-between gap-3 rounded-lg border border-border p-3 text-label"
                    >
                      <span>{monthLabel(month.month)}</span>
                      <span className="font-mono font-semibold">
                        {brl.format(month.projectedResultBrl)}
                      </span>
                    </li>
                  ))}
                </ul>
