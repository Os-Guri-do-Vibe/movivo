...
6. **PII Scrubber inescapável + gate de provedor**: a mensagem do usuário passa pelo scrubber antes de qualquer prompt; `input_snapshot` é pseudonimizado; `dataClass` é `HEALTH` por padrão e só sai para endpoint explicitamente aprovado.
...
- [ ] Ingestão do corpus pelo **dashboard profissional (Sprint 5)** — o RT CREF alimenta metodologia + artigos manualmente; o job de indexação desta US é o backend que a Sprint 5 invoca. Corpus definitivo é pré-requisito de qualidade de lançamento (não bloqueia dev).
...
Duas peças de produto que fecham o Épico 4 e alimentam as próximas sprints. **(1) Handoff + alertas — decisão do fundador (2026-07-30):** o profissional (RT CREF) tem disponibilidade limitada — como qualquer consultoria online, **não se promete resposta humana imediata**. Então o modelo tem **dois níveis distintos**: **(a) Alerta assíncrono ao dashboard** (o caminho comum, **não** é handoff): pedido explícito de falar com o profissional (`PEDIDO_HANDOFF`, direito de contestação/AI Act), FLAG do validador ou intenção fora de escopo → MOVI responde com o **limite honesto** e o sistema **registra um alerta consultável** para o profissional revisar **quando puder** (dashboard, Sprint 5) — sem prometer prazo de retorno. **(b) Handoff de segurança clínica** (reservado a **casos graves de dor / red flag** detectados pelo guardrail da US-3.4): MOVI orienta o usuário a **parar e procurar avaliação médica presencial imediatamente** (a ação é buscar atendimento, não esperar o RT responder) e marca um **alerta prioritário** no dashboard. **Nunca** com diagnóstico/alarme (guardrails). A **decisão de produto crítica:** o **painel do profissional (UI, resposta humana, notificação) é Sprint 5** — nesta sprint **apenas se persiste o estado/alerta** que a Sprint 5 vai consumir. Alexandre valida que o modelo satisfaz o direito de contestação. **(2) Feedback (thumbs) + engajamento:** cada resposta de MOVI vem com **botões de resposta rápida** (`👍 Ajudou` / `👎 Não ajudou`) → PostHog, para medir **CSAT ≥80%** (Épico 4); e instrumentar o evento de **2ª mensagem do usuário no mesmo dia** para medir engajamento conversacional ≥40% (Épico 4). O loop de engajamento (MOVI abre com vitória, 1 pergunta de baixo atrito) já vem dos prompts da US-3.4 — aqui garante-se a **medição**.
...
**TASK-3.6.1 — Alerta assíncrono + handoff de segurança persistidos + mensagem ao usuário (Leonardo + Alexandre).**
Dois níveis (decisão do fundador): **(a) Alerta assíncrono** para `PEDIDO_HANDOFF`, FLAG do validador ou fora de escopo — persistir alerta consultável (motivo + timestamp, sob RLS) e responder ao usuário com o limite honesto, **sem prometer prazo de retorno humano** (é revisão assíncrona, não plantão). **(b) Handoff de segurança clínica** para sinal de dor grave/red flag (guardrail US-3.4) — mensagem que orienta **parar e procurar avaliação médica presencial imediatamente** (sem alarme/diagnóstico, respaldo CREF visível) + alerta **prioritário** persistido. Emitir eventos `handoff_alert` / `handoff_safety` (PostHog). **A tela/resposta humana é Sprint 5** — aqui só o estado/alerta. Alexandre valida o modelo (direito de contestação/AI Act) e que nenhuma mensagem promete SLA humano que não temos.
**Conclusão:** os dois níveis persistem estado consultável + motivo; usuário recebe a mensagem certa nos guardrails (sem promessa de retorno imediato; caso grave orienta atendimento presencial); eventos emitidos; Alexandre valida por escrito; sem UI de painel (Sprint 5).

**TASK-3.6.2 — Captura de feedback (thumbs) + eventos de engajamento (Felipe + Leonardo).**
Capturar 👍/👎 por resposta de MOVI via **botões de resposta rápida** (`👍 Ajudou` / `👎 Não ajudou`) → PostHog (`ai_response_feedback`), para o **CSAT ≥80%** (Épico 4). Instrumentar `whatsapp_user_second_message_same_day` (2ª msg no mesmo dia — engajamento ≥40%, Épico 4) e os eventos de conversa relevantes. Garantir que o feedback é escopado ao usuário e não altera estado de treino.
...
- [ ] Dois níveis (alerta assíncrono / handoff de segurança clínica) persistem estado consultável; nenhuma mensagem promete retorno humano imediato; caso grave orienta atendimento presencial; direito de contestação atendido; painel humano fica para a Sprint 5.
- [ ] Thumbs (CSAT) por **botões de resposta rápida** e evento de 2ª msg/dia instrumentados no PostHog.
